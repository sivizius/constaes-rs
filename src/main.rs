...
{
  let mut a: u32 = aesTE4 [ (( key[4 + 4 * round] >>  0 ) & 0xff ) as usize ] as u32 * 0x01000000;
  let mut b: u32 = aesTE4 [ (( key[4 + 4 * round] >>  8 ) & 0xff ) as usize ] as u32 * 0x00000001;
  let mut c: u32 = aesTE4 [ (( key[4 + 4 * round] >> 16 ) & 0xff ) as usize ] as u32 * 0x00000100;
  let mut d: u32 = aesTE4 [ (( key[4 + 4 * round] >> 24 ) & 0xff ) as usize ] as u32 * 0x00010000;
